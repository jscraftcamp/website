...
describe("Participants JSON file", () => {
  ...
  it("the participants directory only contains JSON files", () => {
    const noJsonFiles = recursiveReaddirSync(srcdir).filter(
      ...
      describe(file, () => {
        ...
        it("may contain a company name", () => {
          if (typeof object.company !== "undefined") {
            assert.equal(
              typeof object.company,
              "string",
              "'company' must be of type string"
            )
            assert.ok(
              object.company.trim().length > 0,
        ...
        it("must contain participation time", () => {
          ...
          assert.ok(
            typeof object.when.friday !== "undefined",
            "'when.friday' is mandatory"
          )
          assert.equal(
            typeof object.when.friday,
            "boolean",
          ...
          assert.equal(
            typeof object.when.saturday,
            "boolean",
            "'when.saturday' must be of type boolean"
          )
        })

        it("must contain a connection to JS statement", () => {
          assert.ok(
            typeof object.what_is_my_connection_to_javascript !== "undefined",
            "'what_is_my_connection_to_javascript' is mandatory"
          )
          assert.equal(
            typeof object.what_is_my_connection_to_javascript,
            "string",
            "'what_is_my_connection_to_javascript' must be of type string"
          )
          assert.ok(
            object.what_is_my_connection_to_javascript.trim().length > 0,
        ...
        it("may contain a what_can_i_contribute statement", () => {
          if (typeof object.what_can_i_contribute !== "undefined") {
            assert.equal(
              typeof object.what_can_i_contribute,
              "string",
              "'what_can_i_contribute' must be of type string"
            )
            assert.ok(
              object.what_can_i_contribute.trim().length > 0,
              "'what_can_i_contribute' must not be empty"
            )
          }
        })
        ...
        it("may contain `allergies`", () => {
          if (typeof object.allergies !== "undefined") {
            assert.equal(
              typeof object.allergies,
              "string",
              "'allergies' must be a string"
            )
          }
        })
        ...
        it("vegan must be boolean", () => {
          assert.equal(
            typeof object.vegan,
            "boolean",
            "'vegan' must be a boolean"
          )
        })

        it("vegetarian must be boolean", () => {
          assert.equal(
            typeof object.vegetarian,
            "boolean",
            "'vegetarian' must be a boolean"
          )
        })
        ...
        it("may contain a twitter handle", () => {
          if (typeof object.twitter !== "undefined") {
            assert.equal(
              typeof object.twitter,
              "string",
              "'twitter' must be of type string"
            )
            assert.ok(
              /^[a-z_]{1}[a-z0-9_]{0,14}$/i.test(object.twitter),
